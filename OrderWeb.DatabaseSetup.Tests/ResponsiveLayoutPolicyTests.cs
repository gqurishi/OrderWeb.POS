...
public sealed class ResponsiveLayoutPolicyTests
{
    ...
    {
        var root = FindRepositoryRoot();
        var sourceFolders = new[] { "Features", "Shared" };
        var violations = sourceFolders
            .SelectMany(folder => Directory.EnumerateFiles(Path.Combine(root, folder), "*.xaml", SearchOption.AllDirectories))
            .Where(path =>
            {
    ...
    [Theory]
    [InlineData("Features/Orders/Views/MoreOptionsDialog.xaml")]
    [InlineData("Features/Payments/Views/CashPaymentDialog.xaml")]
    [InlineData("Features/Payments/Views/CardPaymentDialog.xaml")]
