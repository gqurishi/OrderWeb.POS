	...
	public static MauiApp CreateMauiApp()
	{
		...
		Task.Run(async () =>
		{
			...
			try
			{
				...
				// Start the Mother API independently of background-job registration.
				// Printer/database setup can be slow or fail, but Client POS terminals
				// must still be able to reach the bootstrap endpoint.
				...
				AppDiagnostics.Log("Mother connection startup completed");
			}
			catch (Exception ex)
			{
				AppDiagnostics.LogFatal("Mother connection startup", ex);
			}
			...
			try
			{
				...
				AppDiagnostics.Log("Background sync manager started");
			}
			catch (Exception ex)
			{
				AppDiagnostics.LogFatal("Background sync manager startup", ex);
			}
		});
