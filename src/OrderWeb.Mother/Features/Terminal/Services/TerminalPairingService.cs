...
public static class TerminalPairingService
{
    ...
    {
        // If we're inside a transaction, do NOT run EnsureAttemptsTableAsync here because
        // it may execute DDL (ALTER TABLE) which causes an implicit commit on MySQL and
        // invalidates the active transaction. EnsureAttemptsTableAsync should be called
        // by the caller before beginning a transaction when schema upgrades may be needed.
        if (transaction == null)
        {
            await EnsureAttemptsTableAsync(connection);
        }
