...
public static class ProductionDatabaseCredentialPolicy
{
    ...
    {
        if (!string.Equals(databaseUser?.Trim(), PosDatabaseDefaults.ProductionDatabaseUser, StringComparison.Ordinal))
        {
            return (false, $"The application database user must be '{PosDatabaseDefaults.ProductionDatabaseUser}'; root/admin accounts are not allowed.");
        }
