...
public class CloudOrderService
{
    ...
    private async Task<Order> ConvertCloudOrderToLocalAsync(CloudOrderResponse cloudOrder)
    {
        ...
        var localOrder = new Order
        {
            ...
            // Status and timing
            Status = ParseCloudOrderStatus(cloudOrder.Status),
    ...
    private static OrderStatus ParseCloudOrderStatus(string? status)
    {
        var normalized = (status ?? string.Empty).Trim().Replace("-", string.Empty).Replace("_", string.Empty);
        return Enum.TryParse<OrderStatus>(normalized, true, out var parsed) ? parsed : OrderStatus.New;
    }

    /// <summary>
    /// Searches the complete OrderWeb history without importing, acknowledging, or
    /// automatically printing the returned orders. OrderWeb must support the
    /// history/search parameters on the authenticated pull-orders endpoint.
    /// </summary>
    public async Task<CloudOrderHistorySearchResult> SearchOrderHistoryAsync(
        string query,
        CancellationToken cancellationToken = default)
    {
        var search = query?.Trim().TrimStart('#') ?? string.Empty;
        if (search.Length < 2)
        {
            return CloudOrderHistorySearchResult.Failed("Enter at least 2 characters of an order number or phone number.");
        }
        ...
        try
        {
            ...
            if (config == null)
            {
                return CloudOrderHistorySearchResult.Failed("OrderWeb connection is not configured or is disabled.");
            }

            var endpoint = BuildPullOrdersEndpoint(config.ApiBaseUrl, config.TenantSlug, MaxOrderSyncLimit)
                + $"&include_history=true&search={Uri.EscapeDataString(search)}";
            ...
            if (!response.IsSuccessStatusCode)
            {
                var message = response.StatusCode is HttpStatusCode.BadRequest or HttpStatusCode.NotFound
                    ? "OrderWeb history search is not enabled yet. The cloud API must support include_history=true and search on /api/pos/pull-orders."
                    : $"OrderWeb history search failed ({(int)response.StatusCode}).";
                return CloudOrderHistorySearchResult.Failed(message);
            }
            ...
            if (apiResponse == null || (!apiResponse.Success && !ExtractOrders(apiResponse).Any()))
            {
                return CloudOrderHistorySearchResult.Failed(
                    apiResponse?.Error ?? apiResponse?.Message ?? "OrderWeb returned an invalid history response.");
            }

            // Filter again locally. This prevents an older server that ignores the
            // new search parameters from showing unrelated recent orders.
            var matches = ExtractOrders(apiResponse)
                .Where(order => MatchesHistorySearch(order, search))
                .GroupBy(order => string.IsNullOrWhiteSpace(order.Id) ? order.OrderNumber : order.Id,
                    StringComparer.OrdinalIgnoreCase)
                .Select(group => group.First())
                .OrderByDescending(order => order.CreatedAt)
                .ToList();

            var mapped = new List<Order>(matches.Count);
            foreach (var cloudOrder in matches)
            {
                cancellationToken.ThrowIfCancellationRequested();
                mapped.Add(await ConvertCloudOrderToLocalAsync(cloudOrder));
            }

            return CloudOrderHistorySearchResult.Succeeded(mapped);
        }
        catch (OperationCanceledException)
        {
            throw;
        ...
        {
            AppDiagnostics.Log($"OrderWeb history search failed: {ex.Message}");
            return CloudOrderHistorySearchResult.Failed("Could not reach OrderWeb. Check the internet connection and try again.");
        }
    }

    private static bool MatchesHistorySearch(CloudOrderResponse order, string search)
    {
        var text = search.Trim().TrimStart('#');
        if (ContainsIgnoreCase(order.OrderNumber, text) ||
            ContainsIgnoreCase(order.Id, text) ||
            ContainsIgnoreCase(order.CustomerPhone, text))
        {
            return true;
        }

        var searchDigits = new string(text.Where(char.IsDigit).ToArray());
        var phoneDigits = new string((order.CustomerPhone ?? string.Empty).Where(char.IsDigit).ToArray());
        return searchDigits.Length >= 4 && phoneDigits.Contains(searchDigits, StringComparison.Ordinal);
    }
...
public sealed class CloudOrderHistorySearchResult
{
    public bool Success { get; private init; }
    public IReadOnlyList<Order> Orders { get; private init; } = Array.Empty<Order>();
    public string Message { get; private init; } = string.Empty;

    public static CloudOrderHistorySearchResult Succeeded(IReadOnlyList<Order> orders) => new()
    {
        Success = true,
        Orders = orders,
        Message = orders.Count == 0 ? "No matching OrderWeb orders were found." : $"Found {orders.Count} OrderWeb order(s)."
    };

    public static CloudOrderHistorySearchResult Failed(string message) => new()
    {
        Success = false,
        Message = message
    };
