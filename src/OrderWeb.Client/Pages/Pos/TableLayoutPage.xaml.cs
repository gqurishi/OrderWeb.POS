...
public partial class TableLayoutPage : ContentPage
{
    ...
    private async Task ShowGuestDialogAsync(CachedTable table)
    {
        ...
        {
            await Navigation.PushAsync(SharedOrderEntryPage.ForTableOrLegacy(table, Math.Max(table.Covers, 1)), false);
            return;
        }
    ...
    private async Task OpenTableOrderAsync(int covers)
    {
        ...
        await Navigation.PushAsync(SharedOrderEntryPage.ForTableOrLegacy(_selectedTable, covers), false);
    }
