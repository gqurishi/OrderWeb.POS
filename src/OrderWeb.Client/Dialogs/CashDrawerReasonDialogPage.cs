...
public sealed class CashDrawerFormDialogPage : ContentPage
{
    ...
    public CashDrawerFormDialogPage(string title, string message, string actionText, string accent, string? detailsPlaceholder = null, string? amountLabel = null, bool allowZeroAmount = false)
    {
        _minimumAmount = allowZeroAmount ? 0m : 0.01m;
        BackgroundColor = Color.FromArgb("#99000000");
        ...
        {
            _amount = new Entry { AutomationId = "CashDrawerAmount", Placeholder = "0.00", Keyboard = Keyboard.Numeric, MaxLength = 9, FontSize = 18, TextColor = Color.FromArgb("#202B3D"), PlaceholderColor = Color.FromArgb("#9AA6B8"), IsReadOnly = true };
        ...
        proceed.Clicked += async (_, _) =>
        {
            ...
            if (_amount != null)
            {
                if (!decimal.TryParse(_amount.Text, out var parsed) || parsed < _minimumAmount)
                {
                    await DisplayAlertAsync(
                        "Amount required",
                        _minimumAmount == 0m ? "Enter a valid amount of zero or more." : "Enter an amount greater than zero.",
                        "OK");
                    return;
                }
                amount = parsed;
    ...
    private void AttachTouchKeyboard(Entry entry, string title, bool numeric)
    {
        ...
        tap.Tapped += async (_, _) =>
        {
            ...
            if (numeric)
            {
                ...
                decimal? initial = decimal.TryParse(entry.Text, out var amount) ? amount : null;
                var entered = await keyboard.ShowCurrencyAsync(initial, title, this, _minimumAmount);
                result = entered?.ToString("0.00", System.Globalization.CultureInfo.InvariantCulture);
            }
            else
            {
                ...
                result = await keyboard.ShowAsync(this);
            }
