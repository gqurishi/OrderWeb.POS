...
public partial class MainPage : ContentPage
{
    ...
    private CancellationTokenSource? _liveOrdersRefreshCts;
    ...
    public MainPage()
    {
        ...
        // Do not start until a legacy header clock is shown; SharedAppFrame uses ApplicationHeader.
    ...
    private async void OnMotherAuthoritativeDataChanged(object? sender, MotherDataChangedEventArgs e)
    {
        ...
        await MainThread.InvokeOnMainThreadAsync(async () =>
        {
                ...
                ClientHostAccess.IsMenuRoute(_posSelectedMenu, "restaurant"))
            {
                ...
                {
                    ScheduleRestaurantLayoutRefresh();
                }
    ...
    private void ScheduleRestaurantLayoutRefresh()
    {
        _restaurantLayoutRefreshCts?.Cancel();
        _restaurantLayoutRefreshCts = new CancellationTokenSource();
        var token = _restaurantLayoutRefreshCts.Token;
        _ = DebouncedRestaurantLayoutRefreshAsync(token);
    }

    private async Task DebouncedRestaurantLayoutRefreshAsync(CancellationToken token)
    {
        try
        {
            await Task.Delay(400, token);
            await MainThread.InvokeOnMainThreadAsync(RefreshRestaurantLayoutInPlaceAsync);
        }
    ...
    private void OnMotherConnectionChanged(object? sender, MotherConnectionChangedEventArgs e)
    {
        MainThread.BeginInvokeOnMainThread(() =>
        {
            ...
            // Shared frame loader: only fullscreen-block when cache is cold.
            var warmCache = (_cacheStatus?.Categories ?? 0) > 0 && (_cacheStatus?.Tables ?? 0) > 0;
            if (_activeApplicationFrame is { } frame)
            {
                if (connecting)
                {
                    frame.LoadingMessage = "Connecting to Mother POS…";
                    frame.IsLoading = !warmCache;
                    if (warmCache)
                    {
                        frame.ConnectionStatus = "Reconnect required";
                    }
                }
                else if (e.Connected)
                {
                    frame.IsLoading = false;
                    frame.ConnectionStatus = warmCache ? "Mother online" : "Connecting…";
                    if (!warmCache)
                    {
                        frame.LoadingMessage = "Loading from Mother POS…";
    ...
    /// <summary>
    /// Mother pull into SQLite. Silent by default — never drives fullscreen loader unless
    /// caller passes an explicit progress reporter (cold login / Update All).
    /// </summary>
        ...
        IProgress<MotherSyncProgress>? progress = null)
    {
        ...
        await _configurationVersions.CompareAsync();
        // Do not default to CreateShellSyncProgress — that made every Restaurant open flash a preloader.
        var result = await _operationalSync.PullAllAsync(progress);
        _cachedFloors = await _cache.GetFloorsWithTablesAsync();
        _cacheStatus = await _cache.GetStatusAsync();
        _connectionStatus = result.AnyOk ? "Mother online" : _connectionStatus;
    ...
    /// <summary>Visible sync progress only for cold/empty cache (never for warm silent pulls).</summary>
    private IProgress<MotherSyncProgress> CreateShellSyncProgress() =>
        new Progress<MotherSyncProgress>(report =>
        {
            MainThread.BeginInvokeOnMainThread(() =>
            {
                var warm = (_cacheStatus?.Categories ?? 0) > 0 && (_cacheStatus?.Tables ?? 0) > 0;
                if (warm || _activeApplicationFrame is not { } frame)
                {
                    return;
                }
    ...
    /// <summary>
    /// After login: open till from warm cache when possible; sync in background.
    /// Fullscreen wait only when there is no usable menu/tables cache.
    /// </summary>
    private async Task FinishLoginWithOperationalSyncAsync(LoginSession login)
    {
        ...
        if (await IsOperationalCacheReadyAsync(null, login.Role))
        {
            ShowDashboard();
            StartSilentLiveSync();
            _ = RefreshLoginOperationalSyncInBackgroundAsync(login);
            return;
        }
        ...
        {
            Text = "Loading menu and tables from Mother…",
            FontSize = 18,
        ...
        Root.Children.Add(new VerticalStackLayout
        {
            ...
            Children =
            {
                new ChefLoaderView
                {
                    Mode = ChefLoaderMode.Inline,
                    Size = ChefLoaderSize.Md,
                    Message = "Loading menu and tables",
                    DelayMilliseconds = 0,
                    IsLoading = true,
                    HorizontalOptions = LayoutOptions.Center
                },
    ...
    private async Task RefreshLoginOperationalSyncInBackgroundAsync(LoginSession login)
    {
        try
        {
            ...
            if (!await IsOperationalCacheReadyAsync(sync, login.Role))
            {
                await MainThread.InvokeOnMainThreadAsync(() => ShowOperationalSyncFailed(sync));
                return;
            }
            ...
            await MainThread.InvokeOnMainThreadAsync(() =>
            {
                // Silent success — only surface problems that break the till.
                if (status.Categories <= 0)
                {
                    ShowToast("Menu sync failed: Mother Food Menu has no categories yet. Ordering is empty until you add Food Menu on Mother and Update All.");
                }
                else if (sync is { RequiredOk: false })
                {
                    ShowToast(sync.SummaryMessage());
        ...
        {
            System.Diagnostics.Debug.WriteLine($"Silent login sync failed (using cache): {ex.Message}");
        }
    }
    ...
    {
        if (!_posSidebarOpen && _posSidebarOverlay is null)
        {
            return;
        }

        // Drop hit-testing immediately so the next content tap is never eaten.
        DisablePosSidebarHitTesting();
        var overlay = _posSidebarOverlay;
        var sidebar = _posSidebarView;
        _posSidebarOpen = false;
        ...
        if (overlay != null && Root.Children.Contains(overlay))
        {
    ...
    /// <summary>
    /// Instant sidebar teardown for navigation — no wait for animation before the action runs.
    /// </summary>
    private void DismissPosSidebarForNavigation()
    {
        DisablePosSidebarHitTesting();
        var overlay = _posSidebarOverlay;
        var sidebar = _posSidebarView;
        ...
        // Optional short slide in parallel; overlay already ignores taps.
        _ = FinishPosSidebarVisualCloseAsync(sidebar, overlay);
    }

    private void DisablePosSidebarHitTesting()
    {
        if (_posSidebarOverlay is null)
        {
            return;
        }

        _posSidebarOverlay.InputTransparent = true;
        foreach (var child in _posSidebarOverlay.Children)
        {
    ...
    private async Task FinishPosSidebarVisualCloseAsync(VisualElement? sidebar, Grid overlay)
    {
        try
        {
            if (sidebar != null)
    ...
    private void RunFromPosSidebar(string selectedMenu, Action action)
    {
        _posSelectedMenu = selectedMenu;
        // First tap always works: kill dismiss hit-layer immediately, navigate now.
        DismissPosSidebarForNavigation();
        if (_terminalDisabled)
        {
            ShowTerminalDisabled();
    ...
    private async void OnClientSidebarUpdateAllClicked(object? sender, EventArgs e)
    {
        ...
        var status = await _cache.GetStatusAsync();
        var warmCache = status.Categories > 0 && status.Tables > 0;
        ...
        try
        {
            ...
            if (frame != null)
            {
                if (warmCache)
                {
                    // Keep till usable — toast + connection text, no fullscreen wall.
                    frame.IsLoading = false;
                    frame.ConnectionStatus = "Syncing…";
                    ShowToast("Updating from Mother POS…");
                }
                else
                {
    ...
    private CancellationTokenSource? _silentLiveSyncCts;

    /// <summary>
    /// Keeps SQLite fresh from Mother while logged in. WebSocket is primary;
    /// this is a quiet safety-net pull. Never shows preloaders/toasts.
    /// </summary>
    private void StartSilentLiveSync()
    {
        if (string.Equals(_currentSession?.Role, "Cashier", StringComparison.OrdinalIgnoreCase))
        {
            return;
        ...
        StopSilentLiveSync();
        _silentLiveSyncCts = new CancellationTokenSource();
        var token = _silentLiveSyncCts.Token;
        _ = RunSilentLiveSyncLoopAsync(token);
    }

    private void StopSilentLiveSync()
    {
        try
        {
            _silentLiveSyncCts?.Cancel();
        }
        catch
        {
            // ignore
        ...
        _silentLiveSyncCts?.Dispose();
        _silentLiveSyncCts = null;
    }

    private async Task RunSilentLiveSyncLoopAsync(CancellationToken token)
    {
        // First quiet pull shortly after login (dashboard already shown).
        try
        {
        ...
        while (!token.IsCancellationRequested)
        {
            try
            {
                ...
                if (await _offlinePolicy.IsMotherOnlineAsync())
                {
                    ...
                    await MainThread.InvokeOnMainThreadAsync(() => ApplySilentLiveSyncToOpenPage());
                }
            }
            catch (OperationCanceledException)
            {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Silent live sync: {ex.Message}");
            }
    ...
    private void ApplySilentLiveSyncToOpenPage()
    {
        if (_isViewingOrderScreen || _currentSession is null)
        {
            return;
        }

        if (_posSelectedMenu == "Restaurant" && _restaurantTablesView is not null)
        {
            BindRestaurantTablesView(_restaurantTablesView, _selectedCachedFloor?.Id);
            _restaurantTablesView.ConnectionStatus = _connectionStatus;
            _restaurantTablesView.IsLoading = false;
            return;
        }
    ...
    private async void ShowRestaurantLayout()
    {
        ...
        _isViewingOrderScreen = false;
        await LoadSharedRestaurantLayoutAsync(null, remount: true);
    }

    private async Task RefreshRestaurantLayoutInPlaceAsync()
    {
        if (_posSelectedMenu != "Restaurant" || _isViewingOrderScreen)
        {
            return;
        }

        if (_restaurantTablesView is null)
        {
            await LoadSharedRestaurantLayoutAsync(null, remount: true);
            return;
        }

        await LoadSharedRestaurantLayoutAsync(_selectedCachedFloor?.Id, remount: false);
    }

    private async Task LoadSharedRestaurantLayoutAsync(int? preferredFloorId, bool remount = true)
    {
        RestaurantTablesView tablesView;
        if (!remount && _restaurantTablesView is not null)
        {
            tablesView = _restaurantTablesView;
            tablesView.ConnectionStatus = _connectionStatus;
        }
        else
        {
            tablesView = new RestaurantTablesView
            {
                ConnectionStatus = _connectionStatus,
                IsLoading = false
            };
            ...
            Root.Children.Clear();
            Root.BackgroundColor = Color.FromArgb(PageBackground);
            Root.Children.Add(SharedAppFrame("Restaurant Layout", tablesView, "restaurant"));
        }

        try
        {
            // Cache-first: paint last-good tables immediately — never make the user wait.
            _cachedFloors = await _cache.GetFloorsWithTablesAsync();
            BindRestaurantTablesView(tablesView, preferredFloorId);
            await ApplyFloorBackgroundAsync(tablesView, _selectedCachedFloor);
            tablesView.ConnectionStatus = _connectionStatus;
            tablesView.IsLoading = false;

            var hasTables = _cachedFloors.SelectMany(f => f.Tables).Any();
            if (hasTables)
            {
                // Warm floor: silent Mother pull; patch UI only if data changes.
                _ = SoftRefreshRestaurantFromMotherAsync(tablesView, preferredFloorId ?? _selectedCachedFloor?.Id);
                return;
            }

            // Cold floor only: show inline busy while first hydrate runs.
            tablesView.IsLoading = true;
            var sync = await SyncOperationalDataFromMotherAsync(showToastOnFailure: false);
            _cachedFloors = await _cache.GetFloorsWithTablesAsync();
            _cacheStatus = await _cache.GetStatusAsync();
            BindRestaurantTablesView(tablesView, preferredFloorId ?? _selectedCachedFloor?.Id);
            await ApplyFloorBackgroundAsync(tablesView, _selectedCachedFloor);
            tablesView.ConnectionStatus = _connectionStatus;
            tablesView.IsLoading = false;

            if (!_cachedFloors.SelectMany(f => f.Tables).Any())
            {
        ...
        catch (Exception ex)
        {
            ...
            if (_cachedFloors.SelectMany(f => f.Tables).Any())
            {
                // Keep last-good floor; no toast for soft refresh failures.
                System.Diagnostics.Debug.WriteLine($"Restaurant soft refresh failed: {ex.Message}");
                return;
            }
    ...
    private async Task SoftRefreshRestaurantFromMotherAsync(RestaurantTablesView tablesView, int? preferredFloorId)
    {
        try
        {
            await SyncOperationalDataFromMotherAsync(showToastOnFailure: false);
            if (_posSelectedMenu != "Restaurant" || !ReferenceEquals(_restaurantTablesView, tablesView))
            {
                return;
            }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"Silent restaurant sync failed: {ex.Message}");
        }
    }

    private void BindRestaurantTablesView(RestaurantTablesView tablesView, int? preferredFloorId)
    {
        ...
        var tables = new TableSnapshotDto(version, _cachedFloors.SelectMany(floor => floor.Tables)
            ...
            {
                var isProblem =
                    string.Equals(table.Status, "Reserved", StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(table.SessionStatus, "Cleaning", StringComparison.OrdinalIgnoreCase);
                var hasActiveSession =
                    !string.IsNullOrWhiteSpace(table.CurrentOrderId) ||
                    string.Equals(table.Status, "Occupied", StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(table.SessionStatus, "Occupied", StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(table.SessionStatus, "Payment", StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(table.SessionStatus, "Open", StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(table.SessionStatus, "Active", StringComparison.OrdinalIgnoreCase);
        ...
        tablesView.SetSyncState(RestaurantSyncMode.Live, DateTime.Now);
    }
    ...
    private View TableCard(CachedTable table)
    {
        ...
        var (_, fill, ink) = GetCachedTableColors(table);
    ...
    private static (Color Bg, Color Border, Color Text) GetCachedTableColors(CachedTable table)
    {
        ...
        {
            return (Color.FromArgb("#FEF2F2"), Color.FromArgb("#FEE2E2"), Color.FromArgb(Danger));
        }
        ...
        return (Color.FromArgb("#ECFDF5"), Color.FromArgb("#D1FAE5"), Color.FromArgb(Success));
    }
    ...
    private async Task OpenTableOrderAsync(CachedTable table, int covers)
    {
        ...
        // SharedUI Order Place (same shell as Mother) — not the legacy MainPage ShowOrder layout.
        _isViewingOrderScreen = false;
        try
        {
            var page = new OrderPage(table, covers);
            ClientPageChrome.HideSystemBackChrome(page);
            await Navigation.PushAsync(page, false);
        }
        catch (Exception ex)
    ...
    private async Task SubmitSharedCustomerFlowAsync(CustomerFlowView view, CustomerFlowSubmission submission)
    {
        ...
        try
        {
            ...
            _isViewingOrderScreen = false;
            var page = new OrderPage(result.State, saved.Name, saved.Phone);
            ClientPageChrome.HideSystemBackChrome(page);
            await Navigation.PushAsync(page, false);
        }
        catch (Exception ex)
    ...
    private void ScheduleLiveOrdersRefresh()
    {
        _liveOrdersRefreshCts?.Cancel();
        _liveOrdersRefreshCts = new CancellationTokenSource();
        var token = _liveOrdersRefreshCts.Token;
        _ = DebouncedShowLiveOrdersAsync(token);
    }

    private async Task DebouncedShowLiveOrdersAsync(CancellationToken token)
    {
        try
        {
            await Task.Delay(400, token);
    ...
    private async void ShowLiveOrders(string selectedFilter)
    {
        ...
        RenderLiveOrdersPage(openOrders, selectedFilter, force: true);

        _ = RefreshLiveOrdersFromMotherAsync(selectedFilter);
    }

    private async Task SoftRefreshLiveOrdersAsync()
    {
        var openOrders = await _cache.GetOpenOrderStatesAsync();
        RenderLiveOrdersPage(openOrders, _liveOrderFilter, force: false);
        await RefreshLiveOrdersFromMotherAsync(_liveOrderFilter);
    }

    private async Task RefreshLiveOrdersFromMotherAsync(string selectedFilter)
    {
        try
        {
            ...
            await MainThread.InvokeOnMainThreadAsync(() =>
            {
                ...
                if (!string.Equals(_liveOrderFilter, selectedFilter, StringComparison.OrdinalIgnoreCase))
                {
                    return;
                }

                RenderLiveOrdersPage(openOrders, selectedFilter, force: false);
            });
        }
        catch
        {
    ...
    private void EnsureLegacyClockRunning()
    {
        if (_timeLabel is null || _dateLabel is null)
        {
            _clockTimer.Stop();
            return;
        }

        // Seconds only when legacy restaurant header shows HH:mm:ss; login uses minutes.
        _clockTimer.Interval = TimeSpan.FromSeconds(_useLoginClockFormat ? 30 : 1);
