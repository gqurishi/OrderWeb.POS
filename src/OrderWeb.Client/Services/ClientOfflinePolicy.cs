...
public enum ClientOperation
{
    ...
    /// <summary>Fresh order history from Mother. Requires Mother online.</summary>
    OrderHistory,
    /// <summary>Recent customers cache board from Mother. Requires Mother online.</summary>
    RecentCustomers,
    /// <summary>Read last cached day snapshot of order history while Mother is offline.</summary>
    ViewCachedOrderHistory,
    ...
    AdvanceOrders,
    /// <summary>Bar Inventory stock board via Mother. Requires Mother online.</summary>
    BarInventory
}
...
public sealed class ClientOfflinePolicy
{
    ...
    public OfflineDecision Evaluate(ClientOperation operation, bool motherOnline) => operation switch
    {
        ...
        ClientOperation.ViewCachedOrderHistory => new(true, false, !motherOnline, motherOnline ? "Order history current" : "Mother offline — showing last saved day snapshot if available."),
        ClientOperation.OrderHistory => OnlineOnly(motherOnline, "Order history requires Mother POS. No live history was loaded."),
        ClientOperation.RecentCustomers => OnlineOnly(motherOnline, "Recent customers require Mother POS. No cache was loaded."),
        ...
        ClientOperation.PrintCollectionOrder => OnlineOnly(motherOnline,
            "Mother is offline. Printing runs on Mother — tap Send/Print again when Mother is back."),
        ClientOperation.AdvanceOrders => OnlineOnly(motherOnline,
            "Mother is offline. Advance Orders (and kitchen print) need Mother — retry when Mother is back."),
        ClientOperation.BarInventory => OnlineOnly(motherOnline, "Bar Inventory requires Mother POS. No stock board was loaded."),
        ...
        ClientOperation.PrintAsSuccess => OnlineOnly(motherOnline,
            "Mother is offline. Printing runs on Mother — tap Send/Print again when Mother is back."),
