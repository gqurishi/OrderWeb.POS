...
public partial class MainPage : ContentPage
{
    ...
    private void ShowLogin(bool resetPin = true)
    {
        ...
        var left = new Grid
        {
            ...
            Children =
            {
                new VerticalStackLayout
                {
                    ...
                    Children =
                    {
                        new Label { Text = "POS", FontSize = 56, FontAttributes = FontAttributes.Bold, TextColor = Color.FromArgb(PrimaryAction), HorizontalTextAlignment = TextAlignment.Center },
                        new Label { Text = "Welcome Back!", FontSize = 32, FontAttributes = FontAttributes.Bold, TextColor = Color.FromArgb(MainText), HorizontalTextAlignment = TextAlignment.Center },
                        new Label { Text = CurrentRestaurantName(), FontSize = 24, FontFamily = "InterBold", TextColor = Color.FromArgb(MainText), Opacity = 0.72, HorizontalTextAlignment = TextAlignment.Center, MaximumWidthRequest = 300 }
                    }
                }
            }
        };
        ...
        rightStack.Children.Add(BuildPinDots());
        rightStack.Children.Add(BuildKeypad());
        rightStack.Children.Add(LoginClockButton());

        var right = new Grid
        {
            RowDefinitions =
            {
                new RowDefinition(110),
                new RowDefinition(GridLength.Star)
            },
            Padding = new Thickness(50, 70, 70, 70),
            Children =
            {
                LoginClock(),
                rightStack
    ...
    {
        var dots = new HorizontalStackLayout { Spacing = 24, HorizontalOptions = LayoutOptions.Center, Margin = new Thickness(0, 16, 0, 0) };
        for (var i = 0; i < 4; i++)
        {
    ...
    private Border RoundKey(string text, EventHandler click)
    {
    ...
    private View UserDashboard()
    {
        var layout = new Grid
        {
            RowDefinitions =
            {
                new RowDefinition(90),
                new RowDefinition(GridLength.Star)
            },
            BackgroundColor = Color.FromArgb(PageBackground)
        };

        layout.Children.Add(PosTopHeader(showWelcome: !_posSidebarOpen));
        ...
        SetRow(main, 1);
        return PosDashboardShell(layout);
    }
    ...
    private View AppFrame(string title, View content, bool showSidebar)
    {
        if (UsesPosShell)
        {
            return PosPageFrame(title, content);
        }
        ...
        var grid = new Grid
        {
            ColumnDefinitions =
            {
                new ColumnDefinition(showSidebar ? 270 : 0),
                new ColumnDefinition(GridLength.Star)
            }
        };
        ...
        main.Children.Add(new Grid
        {
            ...
            {
                MenuButton(),
                new Label { Text = title, FontSize = 34, TextColor = Color.FromArgb(MainText), HorizontalTextAlignment = TextAlignment.Center, VerticalTextAlignment = TextAlignment.Center },
                HeaderClock(true)
            }
        });
        SetColumn(((Grid)main.Children[0]).Children[1], 1);
        SetColumn(((Grid)main.Children[0]).Children[2], 2);
    ...
    private View LoginClock()
    {
        ...
        _timeLabel = new Label { FontSize = 23, TextColor = Color.FromArgb(PrimaryAction), HorizontalTextAlignment = TextAlignment.End };
        _dateLabel = new Label { FontSize = 14, FontAttributes = FontAttributes.Bold, TextColor = Color.FromArgb(SecondaryText), HorizontalTextAlignment = TextAlignment.End };
        UpdateClock();
