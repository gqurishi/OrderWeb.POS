...
public sealed class Phase4VerificationPolicyTests
{
    ...
    {
        var source = ReadSource("src/OrderWeb.Mother/Shared/Services/NavigationCoordinator.cs");
    ...
    [Theory]
    [InlineData("src/OrderWeb.Mother/Features/Orders/Pages/OrderPlacementPageSimple.xaml.cs")]
    [InlineData("src/OrderWeb.Mother/Features/Orders/Pages/OrderHistoryPage.xaml.cs")]
    [InlineData("src/OrderWeb.Mother/Features/Reports/Pages/ReportPage.xaml.cs")]
    ...
    {
        var root = FindRepositoryRoot();
        var source = Directory.EnumerateFiles(Path.Combine(root, "src", "OrderWeb.Mother", "Features"), "*.cs", SearchOption.AllDirectories)
